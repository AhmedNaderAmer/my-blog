...
const PostDetails = (props) => {
  ...
  let {data : post,isloading,errMsg} = useFetch(
    `http://localhost:3004/posts/${props.match.params.id}`
    );
    const handleDelete = () => {
      fetch(`http://localhost:3004/posts/${props.match.params.id}`, {
        method: "DELETE",
      }).then(() => {
        props.history.push("/");
      });
    ...
    return (
      ...
      <article className="container post-details">
        ...
        />
        <div className="post-author">By: {post.author ? post.author: "Ahmed"}</div>
